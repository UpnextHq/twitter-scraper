...
const guestTokenCache = new expiry_map_1.default(Math.round(6e4 * 30)); //30 minutes
const getCommonHeaders = () => {
    const REQUEST_USER_AGENT = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36';
    ...
    return {
        ...
        'sec-ch-ua-mobile': '?0',
        'sec-ch-ua-platform': REQUEST_PLATFORMS[Math.floor(Math.random() * REQUEST_PLATFORMS.length)],
        'sec-fetch-dest': 'empty',
        'sec-fetch-mode': 'cors',
        'sec-fetch-site': 'same-origin',
        'user-agent': REQUEST_USER_AGENT,
        'x-csrf-token': crypto_1.default.randomBytes(32).toString('hex'),
        'x-twitter-active-user': 'yes',
        'x-twitter-client-language': 'en',
    };
};
const getGuestToken = (0, p_memoize_1.default)(async () => {
    const { body } = await got_scraping_1.gotScraping.post('https://api.twitter.com/1.1/guest/activate.json', {
        headers: getCommonHeaders(),
        proxyUrl: (0, proxy_1.getProxy)()?.toString('http'),
    });
    return JSON.parse(body).guest_token;
}, { cache: guestTokenCache });
const getTweetDetails = async (tweetId) => {
    const guestToken = await getGuestToken();
    const { body } = await got_scraping_1.gotScraping.get({
...
function getThreadTweetIds(tweetDetails) {
    const threads = [];
    for (const entry of tweetDetails['data']['threaded_conversation_with_injections_v2']['instructions'][0]['entries']) {
...
const getThread = async (tweetId, auth) => {
    const data = await getTweetDetails(tweetId);
    const threadIds = [tweetId, ...getThreadTweetIds(data)];
    const threadTweets = await Promise.all(threadIds.map((threadId) => (0, tweets_1.getTweet)(threadId, auth)));
    ...
    while (startTweet?.isReply && startTweet.inReplyToStatus?.id) {
        const replyTweet = await (0, tweets_1.getTweet)(startTweet.inReplyToStatus?.id, auth);
